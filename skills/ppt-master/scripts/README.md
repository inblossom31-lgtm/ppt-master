...
Native `svg_to_pptx.py` defaults to `baseline`. New free-design/brand-only projects include complete `pptx_layouts` mappings and matching explicit SVG Master/Layout/placeholder metadata; the exporter creates one reusable Layout per contract and validates the finished package. A legacy project whose whole mapping section is absent keeps the compatibility pass: exact z-order-safe shared background/chrome promotion plus coarse root `data-pptx-page-role` families, with filename/id fallbacks. Actual content remains Slide-local in both routes, and no visual-similarity inference is performed. Baseline also prunes unused layouts and maps locked typography/colors into the PowerPoint theme. Use `--pptx-structure flat` for slide-local diagnostics.

Structured baseline and deck/layout template routes use the same explicit Layout compiler. Each complete SVG names its output Layout, repeats reusable Master/Layout preview layers, and marks supported content placeholders. Template strict keeps the selected contract; adaptive may create a new Layout while retaining the Master. The exporter validates cross-slide equality, creates one reusable Layout per key, removes repeated inherited copies, and keeps actual content Slide-local. The complete contract lives in [`references/shared-standards.md`](../references/shared-standards.md#explicit-pptx-master--layout--placeholder-metadata).
