...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **pptx_layouts** / **page_layouts** / **page_charts** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **pptx_layouts (native output Layout contract)**: Write one `P<NN>: <layout_key> | <PowerPoint layout name>` row for every page in every new generated deck. Free-design and brand-only routes remain `mode: baseline`; derive stable keys and picker names from the compositions planned in §IX, without adding `page_layouts`. Name each key after its composition (`timeline-spine`, `kpi-band-trio`), never after a PowerPoint stock role or the page's topic. Template strict uses the selected template SVG's exact key/name; adaptive may introduce a new stable key/name. Legacy preserve uses the exact source key/name. A legacy baseline project whose whole section is absent remains valid and uses page-role compatibility behavior.
   - **Layout-family boundary**: The key identifies the page's composition — its chrome, zone framing, and placeholder set — not its topic and not its slide-local content. Series pages repeating one composition (three directional timetable pages, per-chapter dividers) share one key; a genuinely different composition gets its own key, and the Executor materializes that difference as Layout-layer zone framing (executor-base.md §1.2). Do not split keys because slide-local wording, data, or imagery differ within one composition. Decide families from the authored page plan — never by post-hoc visual clustering, filenames, or content similarity. **Self-check before writing the section** — two failure signals: distinct keys for pages of one composition (topic-splitting; they compile to duplicate Layouts) and one role key covering different compositions (role-clustering; the Layout matches none of them). Re-derive on either.
