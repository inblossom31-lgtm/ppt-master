...
|--------------|---------|-----------|
| Project creation + any triggered Step-3 template selection/installation complete | **Strategist**: Stage 1–2 confirmation + Design Spec | Image_Generator or Executor |
...
Generate Step 3 runs only when explicit template discovery / selection or an
exact workspace root triggers it. Any resulting selection is not a Stage-1
field and must not be reopened here. The sequence is communication first, then
one complete solution + production gate:
...
**Hard rule — Stage-1 source boundary**: Build Stage 1 only from the current user request, source facts, conversation constraints, and project-initialization state. When Step 3 ran, ignore the selected roots, installed/fused template state, template assets/prototypes, and template-owned canvas. Do not load or apply [`strategist-template.md`](./strategist-template.md) until the confirmed Stage-1 contract is available for Stage 2. Early installation is storage preparation, not planning evidence for Stage 1.

> **Execution discipline**: A triggered Step 3 template selection is an earlier independent BLOCKING checkpoint; ordinary free design skips it. Stage 1 and final Stage 2 are the always-on Strategist checkpoints. After final confirmation, proceed without another pause unless spec refinement is enabled.
>
...
> **Default presentation surface — Confirm UI.** Before the first actual confirmation phase, apply [`confirm_ui.md`](../scripts/docs/confirm_ui.md)'s sticky per-run surface decision; its explicit chat branch skips every UI command, and a chat selection after UI launch follows its in-run switch procedure. Chat-question tools alone do not select a branch. A triggered template phase closes selection/installation before Stage 1; ordinary free design opens Stage 1 directly. Then use `<project>/confirm_ui/recommendations.stage1.json` and `.stage2.json`; Stage 2 includes the complete solution and production recommendations. Replace only the active unconfirmed stage, preserve confirmed files, and print the URL plus Stage-1 summary/fallback without treating that handoff as confirmation. Stage 1 writes canonical BCP-47 `primary_language` apart from UI `lang`; Strategist projects it through Design Spec §I to lock communication. Stage 2 carries ≥3 safe / shifted / bold `design_directions`, each bundling visual style, a six-role HEX palette, primary-language heading/body typography plus an English companion only for non-English decks, icons, and conditional image rendering, plus the production fields above. Step 4 retains final confirmation from the selected channel for Design Spec authoring. `confirm_ui.md` owns selection-surface and staged-confirmation lifecycle.
